...
- Added an Ollama LLM provider backed by the official `github.com/ollama/ollama/api` package, with local or remote base URL configuration, streaming responses, tool calling, multimodal image input, and usage accounting.
- Added `ollama` as a supported `goatc` model provider. Ollama defaults to `http://localhost:11434` and does not require an API key.
