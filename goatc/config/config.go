...
func (c *Config) setDefaults() {
	...
	if c.Model.APIKeyEnv == "" {
		...
		case "ollama":
			// Ollama uses its local HTTP endpoint by default.
		}
	}
...
func (c *Config) Validate() error {
	...
	switch strings.ToLower(c.Model.Provider) {
	case "openai", "claude", "anthropic", "gemini", "vertex", "ollama":
	default:
		return fmt.Errorf("unsupported model.provider %q", c.Model.Provider)
	}
	if c.Model.APIKeyEnv == "" && strings.ToLower(c.Model.Provider) != "vertex" && strings.ToLower(c.Model.Provider) != "ollama" {
